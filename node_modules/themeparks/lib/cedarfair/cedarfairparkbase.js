  ...
  FindScheduleDataURL() {
    return this.Cache.Wrap('schedule_url', () => this.GetAPIUrl({
      ...
      if (productDataURL !== undefined) {
        return Promise.resolve(`${productDataURL.purchaseLink.replace(/park-hours[a-zA-Z0-9_-]*\.htm/, '').replace(/\?_=\d+/, '')}js/schedule.js`);
      }
      ...
      return this.GetAPIUrl({
        ...
        if (venueData.details) {
          ...
          if (detail !== undefined) {
            return Promise.resolve(`${detail.description.replace(/park-hours[a-zA-Z0-9_-]*\.htm/, '').replace(/\?_=\d+/, '')}js/schedule.js`);
          }
          return Promise.reject(new Error('Park hours URL has changed, requires themeparks library update'));
        }

        return Promise.reject(new Error('Unable to find schedule data URL'));
      });
